 ...
 *
 * File: tesla.hpp
 * Authors: ppkantorski, pugemon, redraz
 * Description:
 *   This file serves as the core logic for the Ultra Paw Overlay project's custom fork
 ...
 *   usage and configuration information regarding the Ultra-Paw-Overlay.
 *
...
#define ASSERT_EXIT(x) \
    if (R_FAILED(x)) std::exit(1)
#define ASSERT_FATAL(x) \
    if (Result res = x; R_FAILED(res)) fatalThrow(res)
 ...
 */
static const char *CONFIG_FILE = "/config/ultra-paw/config.ini";  // CUSTOM MODIFICATION
...
static void parseOverlaySettings() {
    ...
    u64 decodedKeys = hlp::comboStringToKeys(parsedConfig["ultra-paw"]["key_combo"]);  // CUSTOM MODIFICATION
    if (decodedKeys)
...
[[maybe_unused]] static void updateCombo(u64 keys) {
    ...
    hlp::ini::updateOverlaySettings({{"ultra-paw", {// CUSTOM MODIFICATION
...
static inline int loop(int argc, char **argv) {
    ...
    std::string settingsConfigPath = "sdmc:/config/ultra-paw/config.ini";
    ...
    if (settingsData.count("ultra-paw") > 0 && settingsData["ultra-paw"].count("in_overlay") > 0) {
        inOverlayString = settingsData["ultra-paw"]["in_overlay"];
    } else {
    ...
    if (inOverlayString == "true") {
        inOverlay = true;
        setIniFileValue(settingsConfigPath, "ultra-paw", "in_overlay", "false");
    }
